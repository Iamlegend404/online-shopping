...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get video key
    const vidKey = await validate(
      `${API}/enc-kisskh?text=${encodeURIComponent(id)}&type=vid`,
    );

    // Get video
    const video = await fetch(
      `https://kisskh.do/api/DramaList/Episode/${id}.png?err=false&ts=&time=&kkey=${encodeURIComponent(
        vidKey,
      )}`,
      {
        headers: HEADERS,
    ...
    // Get subtitle key
    const subKey = await validate(
      `${API}/enc-kisskh?text=${encodeURIComponent(id)}&type=sub`,
    );

    // Get subtitles
    const subtitles = await fetch(
      `https://kisskh.do/api/Sub/${id}?kkey=${encodeURIComponent(subKey)}`,
      {
        headers: HEADERS,
        cache: "no-store",
      },
    ...
    let decryptedSubtitle: string | null = null;

    if (Array.isArray(subtitles) && subtitles.length > 0 && subtitles[0]?.src) {
      decryptedSubtitle = await fetch(
        `${API}/dec-kisskh?url=${encodeURIComponent(subtitles[0].src)}`,
        {
          cache: "no-store",
        },
      ).then((r) => r.text());
